...
const ProductComponent: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch('http://localhost:8000/api/products') // <-- Replace with your actual API URL
        ...
        const mappedProducts: Product[] = data.map((item: any) => ({
          ...
          originalPrice: parseFloat(item.compare_price) || parseFloat(item.price) || 0,
          discount: item.compare_price
            ? Math.round(((parseFloat(item.compare_price) - parseFloat(item.price)) / parseFloat(item.compare_price)) * 100)
            : 0,
          category: item.category || 'uncategorized',
          inStock: item.stock > 0 && item.is_active,
          features: ['Feature 1', 'Feature 2'], // Placeholder values
          rating: 4.5, // Placeholder value
          reviews: 20, // Placeholder value
        }))
  ...
  Cookies.set('wishlist', JSON.stringify(updatedWishlist), { expires: 7 }) // expires in 7 days
}
